import { CLEAR_COLORS, DIFFICULTY_COLORS } from "../config/app-config.js";
import {
  drawThemedBackground,
  drawNeonTitle,
  drawStatPanel,
  createSummaryItemsForScoreGrid
} from "./common-canvas.js";

export async function renderScoreGridPage({
  canvas,
  data: songs,
  pageDefinition,
  uiConfig,
  ...
  drawStatPanel(
    ctx,
    {
      ...
      items: createSummaryItemsForScoreGrid(songs, CLEAR_COLORS)
    },
    uiConfig
  );
...
async function drawJacket(ctx, jacketData, jacketRepository, uiConfig) {
  ...
  if (song.score > 0) {
    if (song.clear === "per") {
      ...
      const scoreHeight = size * 0.3;
      const scoreY = y + size - scoreHeight;

      const scoreGrad = ctx.createLinearGradient(0, scoreY, 0, y + size);
      scoreGrad.addColorStop(0, "rgba(0,0,0,0.6)");
      scoreGrad.addColorStop(1, "rgba(0,0,0,0.9)");
      ctx.fillStyle = scoreGrad;
      ctx.fillRect(x, scoreY, size, scoreHeight);

      ctx.fillStyle = CLEAR_COLORS[song.clear] || "#fff";
      ctx.font = uiConfig.fonts.score;
      ctx.textAlign = "center";
      ctx.textBaseline = "middle";
      ctx.fillText(
        Math.floor(song.score / 1000),
        x + size / 2,
        y + size * 0.85
      );
    }
